...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_purchase'])) {
    ...
    if (!empty($po_num) && !empty($supplier_id) && !empty($product_name) && $qty > 0) {
        ...
        $stmt->execute();
        $stmt->close();
        header("Location: admin.php?page=supplier_purchases&success=Purchase added successfully");
        exit;
    } else {
        header("Location: admin.php?page=supplier_purchases&error=Invalid input");
        exit;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_purchase'])) {
    ...
    header("Location: admin.php?page=supplier_purchases&success=Purchase updated successfully");
    exit;
}
...
if (isset($_GET['delete'])) {
    ...
    header("Location: admin.php?page=supplier_purchases&success=Purchase deleted successfully");
    exit;
}
